...
export default function PrismNodePage() {
  ...
  const fetchVitals = useCallback(async (tk: string, did: string) => {
    try {
      ...
      const pulseRes = await fetch(`/api/v1/physio/pulse/readings/${did}?limit=60`, { headers: { Authorization: `Bearer ${tk}` } })
      // Also try legacy PPG readings
      const ppgRes = await fetch(`/api/v1/physio/readings/${did}?sensor_type=ppg&limit=60`, { headers: { Authorization: `Bearer ${tk}` } })
  ...
  const fetchSleep = useCallback(async (tk: string, did: string) => {
    try {
      const res = await fetch(`/api/v1/physio/sleep/${did}?limit=30`, { headers: { Authorization: `Bearer ${tk}` } })
  ...
  const fetchStatus = useCallback(async (tk: string, did: string) => {
    try {
      const res = await fetch(`/api/v1/physio/status/${did}`, { headers: { Authorization: `Bearer ${tk}` } })
