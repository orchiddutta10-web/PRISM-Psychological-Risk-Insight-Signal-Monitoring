...
class StateTracker:
    ...
    def update(self, face_feats: Dict[str, Any], pose_feats: Dict[str, Any], motion_feats: Dict[str, Any], voice_feats: Dict[str, Any], timestamp: float) -> Dict[str, Any]:
        ...
        # Prune old blinks
        while self.blink_timestamps and (timestamp - self.blink_timestamps[0] > self.history_window_sec):
            self.blink_timestamps.popleft()
            
        # Blinks per minute (normalized if window < 60s)
        time_observed = min(timestamp - self.first_seen_time if self.first_seen_time > 0 else 0, self.history_window_sec)
        if time_observed > 5.0:  # Need at least 5s to estimate BPM
            state["blink_frequency_bpm"] = round((len(self.blink_timestamps) / time_observed) * 60.0, 1)
        ...
        while self.motion_spikes and (timestamp - self.motion_spikes[0] > self.history_window_sec):
        ...
        while self.audio_speech_windows and (timestamp - self.audio_speech_windows[0][0] > self.history_window_sec):
        ...
        if self.audio_speech_windows:
            speaking_frames = sum(1 for _, active in self.audio_speech_windows if active)
