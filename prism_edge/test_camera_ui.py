...
def main():
    ...
    try:
        while True:
            ...
            cv2.putText(display_frame, "PRISM Edge Phase 5/6 Test", (10, 30), font, 0.7, (255, 255, 255), 2)
            
            # Motion Overlay
            motion_mag = motion_data.get("motion_magnitude", 0)
            is_idle = motion_data.get("is_idle", True)
            status_text = "IDLE" if is_idle else "ACTIVE"
            cv2.putText(display_frame, f"Motion Mag: {motion_mag:.3f}", (10, 70), font, 0.6, color, 2)
            cv2.putText(display_frame, f"Status: {status_text}", (10, 100), font, 0.6, (0, 0, 255) if is_idle else color, 2)
