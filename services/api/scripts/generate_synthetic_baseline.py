...
def generate_normal_day_telemetry(subject_id, date, is_api=False):
    ...
    events.append({
        "subject_id": subject_id,
        "modality": "typing",
        "value": {"delay_index": random.uniform(0.9, 1.1), "correction_rate_variance": random.uniform(0.05, 0.15), "is_synthetic": True},
        "confidence": 0.90,
    ...
    events.append({
        "subject_id": subject_id,
        "modality": "app_usage",
        "value": {"late_night_hours": random.uniform(0.1, 0.8), "baseline_hours": 1.0, "is_synthetic": True},
        "confidence": 0.95,
    ...
    events.append({
        "subject_id": subject_id,
        "modality": "gsr",
        "value": {"gsr_microsiemens": random.uniform(1.0, 2.5), "is_synthetic": True},
        "confidence": 0.90,
...
def run_generator():
    ...
    parser.add_argument("--days", type=int, default=14, help="Number of baseline days (14-30)")
    parser.add_argument("--perturb", action="store_true", help="Inject labeled anomalies in last days")
    parser.add_argument("--api", action="store_true", help="Push over API instead of direct DB seeding")
    parser.add_argument("--api_url", default="http://localhost:8000/api/v1/events/ingest/unified", help="Ingestion API endpoint")
    ...
    for i in range(args.days):
        ...
        if args.perturb and (args.days - i) <= 4:
            ...
            if day_offset == 4:
                day_events = generate_perturbed_telemetry(args.subject_id, current_date, "late_night_spike")
            elif day_offset == 3:
                day_events = generate_perturbed_telemetry(args.subject_id, current_date, "social_withdrawal")
            elif day_offset == 2:
                day_events = generate_perturbed_telemetry(args.subject_id, current_date, "risky_app")
            else:
                day_events = generate_perturbed_telemetry(args.subject_id, current_date, "physio_stress")
        else:
        ...
        for ev in all_events:
            try:
                ...
                if res.status_code == 200:
                    ...
                    print(f"Failed to ingest event {ev['modality']}: Status {res.status_code} - {res.text}")
            except Exception as ex:
                print(f"API Connection error: {ex}")
        print(f"API Ingestion complete: {success_count}/{len(all_events)} events ingested.")
        
    else:
        if not SessionLocal:
            print("Error: app.database and models not importable. Cannot run direct seeding.")
            sys.exit(1)
            
        db = SessionLocal()
        try:
            # Verify subject exists
            device = db.query(models.ChildDevice).filter(models.ChildDevice.id == args.subject_id).first()
            if not device:
                print(f"Error: Child device with ID '{args.subject_id}' does not exist in DB.")
                sys.exit(1)
            ...
            db.commit()
            print(f"Direct DB seeding complete: {inserted_count} events added to unified_events.")
