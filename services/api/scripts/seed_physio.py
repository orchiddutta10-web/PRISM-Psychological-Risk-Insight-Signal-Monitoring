...
def seed():
    ...
    if not token:
        ...
        if (
            ...
            and "already registered" not in r.text.lower()
        ):
    ...
    # 3. Consent grants
    r = requests.post(
        f"{API}/consent/grants/{device_id}",
    ...
    r = requests.post(
        f"{API}/consent",
        json={"signal_type": "gsr", "consent_copy_version": 1, "granted": True},
        headers=dev_headers,
    )
    ...
    r_ppg = requests.get(
        f"{API}/physio/readings/{device_id}?sensor_type=ppg&limit=3", headers=headers
    )
    r_gsr = requests.get(
        f"{API}/physio/readings/{device_id}?sensor_type=gsr&limit=3", headers=headers
    )
