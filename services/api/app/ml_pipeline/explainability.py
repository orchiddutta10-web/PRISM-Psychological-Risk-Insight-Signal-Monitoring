...
class ExplainabilityEngine:
    ...
    def generate_explanations(self, instance: pd.DataFrame, top_k: int = 3) -> dict:
        ...
        feature_impacts = [
            (name, val) for name, val in zip(feature_names, instance_shap)
        ]
        ...
        for name, impact in top_factors:
            ...
            if "Sleep" in readable_name:
                plain_language.append(
                    f"Significant {direction} risk due to recent sleep patterns (Value: {val:.1f})."
                )
            elif "Variance" in readable_name:
                plain_language.append(
                    f"High volatility in {readable_name.lower().replace('variance', 'routine')} detected."
                )
            else:
                plain_language.append(
                    f"Unusual {readable_name.lower()} contributed to this score."
                )
