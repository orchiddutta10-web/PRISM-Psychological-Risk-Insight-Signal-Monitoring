...
class RAGCompanionEngine:
    ...
    def _build_context_prompt(
        ...
        clinical_docs = self._retrieve_knowledge(f"Stress sleep withdrawal indicators: {guardian_question}")
        
        # 2. Format citations
        citations = "\n".join([f"- {doc['content']} (Source: {doc['source']})" for doc in clinical_docs])
    ...
    def generate_response(self, guardian_question: str, telemetry: Dict[str, float], context: Dict[str, Any]) -> str:
