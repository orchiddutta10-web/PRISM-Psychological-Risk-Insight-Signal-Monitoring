...
class FeatureStore:
    ...
    def get_features_for_device(
        self, device_id: str, target_time: datetime = None
    ) -> Dict[str, float]:
        """
        ...
        sleep_z_score = (
            (current_sleep - avg_sleep) / (np.sqrt(sleep_variance) + 1e-5)
            if avg_sleep > 0
            else 0.0
        )
    ...
    def generate_training_dataset(self, device_ids: List[str]) -> pd.DataFrame:
        ...
        for did in device_ids:
            ...
            for i in range(14):
                ...
                feats["is_anomalous"] = (
                    1 if abs(feats["sleep_z_score_24h"]) > 2.0 else 0
                )
                rows.append(feats)
