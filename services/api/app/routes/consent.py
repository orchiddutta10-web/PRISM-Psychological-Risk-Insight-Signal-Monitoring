...
def get_consent_records(
    ...
    records = db.query(models.ConsentRecord).filter(
        models.ConsentRecord.device_id == device_id
    ).all()
...
def update_consent_grant(
    ...
    grant = db.query(models.ConsentGrant).filter(
    ...
    return {"status": "success", "modality": grant.modality, "is_granted": grant.is_granted}
...
def get_consent_grants(
    ...
    """
    auth.verify_guardian_device_access(current_guardian, device_id, db)
    grants = db.query(models.ConsentGrant).filter(
        models.ConsentGrant.subject_id == device_id
    ).all()
