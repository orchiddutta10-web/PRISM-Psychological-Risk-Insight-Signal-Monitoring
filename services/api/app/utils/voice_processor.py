...
MODEL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "resources", "voice_model.joblib")
voice_clf = None
...
def extract_speaker_embedding(audio_bytes: bytes) -> list[float]:
    ...
    Uses a deterministic projection over the audio stream to simulate a pretrained model
    (e.g., Resemblyzer) without heavy native dependencies.
    """
    # Use MD5/SHA256 to seed the projection so same clip always gives same embedding
...
def extract_acoustic_features(audio_bytes: bytes) -> np.ndarray:
    ...
    if len(audio_bytes) > 1000:
        try:
            ...
            chroma = librosa.feature.chroma_stft(y=data, sr=samplerate, n_chroma=12).mean(axis=1)
            mel = librosa.feature.melspectrogram(y=data, sr=samplerate, n_mels=128).mean(axis=1)
