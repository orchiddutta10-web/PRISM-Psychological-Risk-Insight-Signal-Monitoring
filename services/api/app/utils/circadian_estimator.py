...
def infer_sleep_windows(db: Session):
    ...
    for device in devices:
        events = (
            ...
            .order_by(models.UnifiedEvent.timestamp.asc())
            .all()
        )
        ...
        if longest_gap_start and longest_gap_end:
            ...
            existing = (
                ...
                .filter(
                    ...
                    models.SleepWindow.estimated_start
                    >= longest_gap_start - timedelta(hours=2),
                    models.SleepWindow.estimated_start
                    <= longest_gap_start + timedelta(hours=2),
                )
                .first()
            ...
            if not existing:
                ...
                hr_vals = [
                    e.value.get("heart_rate_bpm")
                    for e in sleep_events
                    if e.modality == "ppg" and "heart_rate_bpm" in e.value
                ]
                gsr_vals = [
                    e.value.get("gsr_microsiemens")
                    for e in sleep_events
                    if e.modality == "gsr" and "gsr_microsiemens" in e.value
                ]
                ...
                if reg_index < 0.5:
                    ...
                    alert.contributing_factors = [
                        f"Circadian regularity index dropped to {int(reg_index*100)}%."
                    ]
                    db.add(alert)
                    db.commit()
