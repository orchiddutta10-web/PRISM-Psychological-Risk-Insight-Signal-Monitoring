...
def test_no_raw_content_in_schema():
    ...
    forbidden_keywords = {"content", "text", "message", "body", "audio", "video", "image", "photo", "screen", "screenshot"}
    for table_name, table in Base.metadata.tables.items():
        for column in table.columns:
            col_name = column.name.lower()
            for kw in forbidden_keywords:
                assert kw not in col_name, f"Violation: Forbidden field '{col_name}' containing '{kw}' found in table '{table_name}'."
...
def test_consent_management_authz():
    ...
    client.post(
        "/api/v1/auth/register",
        json={"full_name": "Sarah", "email": "sarah@example.com", "password": "password123"}
    )
    ...
    response = client.post(
        "/api/v1/consent",
        headers={"Authorization": f"Bearer {guardian_token}"},
        json={"signal_type": "location", "granted": True, "consent_copy_version": "1.0"}
    )
    assert response.status_code == 401
    ...
    client.post(
        "/api/v1/auth/register",
        json={"full_name": "Stranger", "email": "stranger@example.com", "password": "password123"}
    )
...
def test_telemetry_ingestion_and_encryption():
    ...
    db = TestingSessionLocal()
    saved_event = db.query(models.RawSignalEvent).filter(models.RawSignalEvent.id == event_id).first()
...
def test_scenario_a_late_night_spike():
    ...
    client.post("/api/v1/auth/register", json={"full_name": "Sarah", "email": "sarah@example.com", "password": "password123"})
    res_login = client.post("/api/v1/auth/login", json={"email": "sarah@example.com", "password": "password123"})
    guardian_token = res_login.json()["access_token"]
    res_dev = client.post("/api/v1/auth/device", headers={"Authorization": f"Bearer {guardian_token}"}, json={"name": "Tommy", "platform": "android", "device_token": "tok"})
    device_id = res_dev.json()["device"]["id"]
    device_jwt = res_dev.json()["device_jwt_token"]

    client.post("/api/v1/consent", headers={"Authorization": f"Bearer {device_jwt}"}, json={"signal_type": "app_usage", "granted": True, "consent_copy_version": "1.0"})
    client.post("/api/v1/consent", headers={"Authorization": f"Bearer {device_jwt}"}, json={"signal_type": "location", "granted": True, "consent_copy_version": "1.0"})

    client.post(
        "/api/v1/events/ingest",
        headers={"Authorization": f"Bearer {device_jwt}"},
        json={"device_id": device_id, "signal_type": "location", "metadata": {"steps": 1500}}
    )

    res = client.post(
        "/api/v1/events/ingest",
        headers={"Authorization": f"Bearer {device_jwt}"},
        json={"device_id": device_id, "signal_type": "app_usage", "metadata": {"late_night_hours": 3.5, "baseline_hours": 1.0}}
    )
    assert res.status_code == 200

    res_alerts = client.get(f"/api/v1/events/alerts/{device_id}", headers={"Authorization": f"Bearer {guardian_token}"})
    ...
    assert any("Late-night app usage rose" in f for f in latest_alert["contributing_factors"])
    assert any("Daily movement dropped" in f for f in latest_alert["contributing_factors"])
...
def test_scenario_b_social_withdrawal_and_fatigue():
    ...
    client.post("/api/v1/consent", headers={"Authorization": f"Bearer {device_jwt}"}, json={"signal_type": "typing", "granted": True, "consent_copy_version": "1.0"})
    client.post("/api/v1/consent", headers={"Authorization": f"Bearer {device_jwt}"}, json={"signal_type": "location", "granted": True, "consent_copy_version": "1.0"})

    client.post(
        "/api/v1/events/ingest",
        headers={"Authorization": f"Bearer {device_jwt}"},
        json={"device_id": device_id, "signal_type": "location", "metadata": {"steps": 2000}}
    )

    client.post(
        "/api/v1/events/ingest",
        headers={"Authorization": f"Bearer {device_jwt}"},
        json={"device_id": device_id, "signal_type": "typing", "metadata": {"delay_index": 1.4}}
    )
    ...
    assert any("Daily movement dropped" in f for f in latest_alert["contributing_factors"])
    assert any("delay index increased by 40" in f for f in latest_alert["contributing_factors"])
...
def test_scenario_c_new_app_risk():
    ...
    assert any("Installed risky app: com.anonymous.chat" in f for f in latest_alert["contributing_factors"])
    assert any("Late-night app usage rose to 3.0h" in f for f in latest_alert["contributing_factors"])
...
def test_model_performance_and_latency_load():
    ...
    for _ in range(50):
        client.post(
            "/api/v1/events/ingest",
            headers={"Authorization": f"Bearer {device_jwt}"},
            json={"device_id": device_id, "signal_type": "location", "metadata": {"steps": 12000}}
        )
    end_time = time.time()
...
def test_voice_checkin():
    ...
    db = TestingSessionLocal()
    consent = models.ConsentGrant(subject_id=device_id, modality="voice", is_granted=True)
    db.add(consent)
    db.commit()
    db.close()
    ...
    response = client.post(
        "/api/v1/voice/checkin",
        headers={"Authorization": f"Bearer {device_jwt}"},
        files={"audio": ("checkin.wav", sample_a, "audio/wav")} # Same bytes -> similarity is 1.0
    )
    assert response.status_code == 200
    ...
    response = client.post(
        "/api/v1/voice/checkin",
        headers={"Authorization": f"Bearer {device_jwt}"},
        files={"audio": ("other_voice.wav", sample_b, "audio/wav")} # Diff bytes -> similarity < 0.75
    )
    assert response.status_code == 403
    ...
    db = TestingSessionLocal()
    retention_consent = models.ConsentGrant(subject_id=device_id, modality="voice_retention", is_granted=True)
...
def test_companion_chat():
    ...
    db = TestingSessionLocal()
    consent = models.ConsentGrant(subject_id=device_id, modality="companion_chat", is_granted=True)
    db.add(consent)
    db.commit()
    db.close()
    ...
    assert "I'm an AI companion, not a licensed therapist or doctor" in response.json()["disclosure_banner"]
    ...
    assert "text HOME to 741741" in response.json()["response"] # crisis hotline details
...
def test_meta_webhooks():
    ...
    response = client.get(
        ...
        params={"hub.mode": "subscribe", "hub.challenge": "12345challenge", "hub.verify_token": "prism_verify_secret"}
    )
    assert response.status_code == 200
    ...
    response = client.get(
        "/api/v1/companion/webhook/meta",
        params={"hub.mode": "subscribe", "hub.challenge": "12345challenge", "hub.verify_token": "wrong_token"}
    )
    assert response.status_code == 403
        ...
        "entry": [{
            ...
            "changes": [{
                "value": {
                    ...
                    "messages": [{
                        "from": device_id,
                        "text": {"body": "How can I set structured daily goals?"}
                    }]
                },
                "field": "messages"
            }]
...
def test_synthetic_generator_and_sleep_estimator():
    ...
    guardian = models.Guardian(full_name="Parent", email="parent@example.com", password_hash="hash")
    db.add(guardian)
    db.commit()
    db.refresh(guardian)
    
    device = models.ChildDevice(guardian_id=guardian.id, name="Kid", platform="ios", device_token="token123")
    db.add(device)
    db.commit()
    db.refresh(device)
    ...
    sleep_windows = db.query(models.SleepWindow).filter(models.SleepWindow.subject_id == device.id).all()
...
def test_risk_registry_hits_and_companion_crisis():
    ...
    client.post("/api/v1/auth/register", json={"full_name": "Dave", "email": "dave@example.com", "password": "password123"})
    res_login = client.post("/api/v1/auth/login", json={"email": "dave@example.com", "password": "password123"})
    guardian_token = res_login.json()["access_token"]
    res_dev = client.post("/api/v1/auth/device", headers={"Authorization": f"Bearer {guardian_token}"}, json={"name": "Alice", "platform": "ios", "device_token": "token456"})
    device_id = res_dev.json()["device"]["id"]
    device_jwt = res_dev.json()["device_jwt_token"]
    
    # Enable consent for companion_chat and browse_metadata
    db.add(models.ConsentGrant(subject_id=device_id, modality="companion_chat", is_granted=True))
    db.add(models.ConsentGrant(subject_id=device_id, modality="browse_metadata", is_granted=True))
    db.commit()
    ...
    res_ingest = client.post(
        ...
        json={
            ...
            "value": {"search_query": "how to cut yourself and end it", "is_synthetic": True},
            "confidence": 1.0
        }
    )
    assert res_ingest.status_code == 200
    
    hits = db.query(models.RiskRegistryHit).filter(models.RiskRegistryHit.subject_id == device_id).all()
    assert len(hits) > 0
