...
def run_crisis_load_test():
    ...
    res_dev = client.post("/api/v1/auth/device", headers={"Authorization": f"Bearer {guardian_token}"}, json={
        "name": "Tommy LoadTest",
        "platform": "ios",
    ...
    for signal in ["location", "typing", "app_usage"]:
        client.post("/api/v1/consent", headers={"Authorization": f"Bearer {device_jwt}"}, json={
            "signal_type": signal,
            "granted": True,
    ...
    for i in range(300):
        dataset.append({
            ...
            "metadata": {"steps": 12000, "delay_index": 1.0} if i % 2 == 0 else {"delay_index": 1.0, "correction_rate_variance": 0.01}
        })
    ...
    for _ in range(50):
        dataset.append({
            ...
            "metadata": {"steps": 1500} # stationary
        })
        dataset.append({
    ...
    for _ in range(50):
        ...
        dataset.append({
            "signal_type": "typing",
            "metadata": {"delay_index": 1.4, "correction_rate_variance": 0.04} # slow typing
        })
    ...
    print(f"Dataset compiled. Running load testing of {len(dataset)} telemetry ingestions...")
    ...
    total_alerts = db.query(models.Alert).filter(models.Alert.device_id == device_id).all()
    ...
    recall = (detected_red_count / expected_red_count) * 100 if expected_red_count > 0 else 100
...
"""
    ...
    print("Load Test Finished! Report generated at C:\\Users\\Jyotishmoy Gogoi\\.gemini\\antigravity-cli\\brain\\792c2d21-a659-4a76-b30f-4cff702a6227\\prism_load_test_report.md")
