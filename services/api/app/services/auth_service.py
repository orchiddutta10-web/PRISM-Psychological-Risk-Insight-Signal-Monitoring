...
class AuthService:
    @staticmethod
    def register_guardian(guardian_in: schemas.GuardianCreate, db: Session, ip_address: str = None) -> models.Guardian:
        existing_guardian = db.query(models.Guardian).filter(models.Guardian.email == guardian_in.email).first()
    ...
    @staticmethod
    def login_guardian(login_data: schemas.LoginRequest, db: Session, ip_address: str = None) -> dict:
        guardian = db.query(models.Guardian).filter(models.Guardian.email == login_data.email).first()
        if not guardian or not auth.verify_password(login_data.password, guardian.password_hash):
            audit.log_audit_event(
                db,
        ...
        if settings.ENV.lower() == "production":
            ...
            print(f"--- [MFA OTP] Sent code {code} to email/phone for {guardian.email} ---")
    ...
    @staticmethod
    def verify_mfa(payload: schemas.VerifyMFARequest, db: Session, ip_address: str = None) -> dict:
        try:
            data = jwt.decode(payload.mfa_token, settings.JWT_SECRET, algorithms=[settings.JWT_ALGORITHM])
    ...
    @staticmethod
    def register_device(device_in: schemas.ChildDeviceCreate, guardian_id: str, db: Session, ip_address: str = None) -> dict:
        existing_device = db.query(models.ChildDevice).filter(models.ChildDevice.device_token == device_in.device_token).first()
        if existing_device:
            existing_device.last_seen = datetime.now(timezone.utc)
            db.commit()
            db.refresh(existing_device)

            device_jwt = auth.create_access_token(data={"sub": existing_device.id, "type": "device"})
            return {
                "device": existing_device,
                "device_jwt_token": device_jwt
            }
    ...
    @staticmethod
    def send_otp(req: schemas.SendOTPRequest, db: Session, ip_address: str = None) -> dict:
    ...
    @staticmethod
    def verify_otp(req: schemas.VerifyOTPRequest, db: Session, ip_address: str = None) -> dict:
        phone = req.phone_number.strip()
        code = req.code.strip()
        ...
        guardian = db.query(models.Guardian).filter(models.Guardian.email == mapped_email).first()
    ...
    @staticmethod
    def register_otp_guardian(req: schemas.RegisterOTPRequest, db: Session, ip_address: str = None) -> dict:
        phone = req.phone_number.strip()
        name = req.full_name.strip()
        mapped_email = f"{phone}@prism-otp.org"

        existing = db.query(models.Guardian).filter(models.Guardian.email == mapped_email).first()
        if existing:
